...
✨ 매치 점수가 높을수록 관련성 높음
    ...
    def initUI(self):
        ...
        tab1_layout.addLayout(search_conditions_layout)
        ...
        tab2_layout.addWidget(control_panel)

        # 탭 3: 고급 스크리닝 (새로 추가)
        tab3 = self.create_advanced_screening_tab()
    ...
    def create_advanced_screening_tab(self):
        ...
        info_label.setStyleSheet("color: #666; background-color: #f0f0f0; padding: 8px; border-radius: 4px;")
        layout.addWidget(info_label)

        # 프리셋 선택 영역
        preset_group = QGroupBox("⚡ 빠른 프리셋 선택")
        preset_layout = QHBoxLayout()
        preset_layout.setSpacing(10)

        self.preset_combo = QComboBox()
        self.preset_combo.addItems(["사용자 정의", "🛡️ 안전형", "⚖️ 균형형", "🚀 공격형"])
        self.preset_combo.currentTextChanged.connect(self.apply_advanced_preset)
        preset_layout.addWidget(QLabel("전략 프리셋:"))
        ...
        # 고급 조건 그룹박스
        conditions_group = QGroupBox("🎯 고급 스크리닝 조건")
        conditions_layout = QGridLayout()
        conditions_layout.setSpacing(10)

        # 다중 시간대 분석
        self.adv_multi_timeframe = QCheckBox("📅 다중 시간대 추세 확인 (일봉+주봉+월봉)")
        self.adv_multi_timeframe.setToolTip("일봉, 주봉, 월봉 모두 상승 추세인 종목만 선택")
        conditions_layout.addWidget(self.adv_multi_timeframe, 0, 0, 1, 2)

        # 시장 강도 필터
        self.adv_market_strength = QCheckBox("💪 시장 강도 필터")
        self.adv_market_strength.setToolTip("시장이 강세일 때만 매수 신호 발생")
        conditions_layout.addWidget(self.adv_market_strength, 1, 0, 1, 2)

        # 상대 강도 비교 (Phase 3 - 활성화!)
        self.adv_relative_strength = QCheckBox("📊 상대 강도 비교 (섹터 대비)")
        self.adv_relative_strength.setToolTip("같은 섹터 내 다른 종목보다 강한 종목만 선택 (미국 종목만 지원)")
        self.adv_relative_strength.setEnabled(True)  # Phase 3에서 활성화
        conditions_layout.addWidget(self.adv_relative_strength, 2, 0, 1, 2)

        # 볼륨 프로파일 (Phase 3 - 활성화!)
        self.adv_volume_profile = QCheckBox("📈 볼륨 프로파일 분석")
        self.adv_volume_profile.setToolTip("거래량 패턴 분석 - VAL/VAH/POC 기반")
        self.adv_volume_profile.setEnabled(True)  # Phase 3에서 활성화
        conditions_layout.addWidget(self.adv_volume_profile, 3, 0, 1, 2)

        # 지지/저항선 감지 (Phase 3 - 활성화!)
        self.adv_support_resistance = QCheckBox("🎯 지지/저항선 자동 감지")
        self.adv_support_resistance.setToolTip("주요 가격대 지지/저항선 자동 감지")
        self.adv_support_resistance.setEnabled(True)  # Phase 3에서 활성화
        conditions_layout.addWidget(self.adv_support_resistance, 4, 0, 1, 2)
        ...
        # 다중 시간대 설정
        settings_layout.addWidget(QLabel("다중 시간대 모드:"), 0, 0)
        self.mtf_mode_combo = QComboBox()
        self.mtf_mode_combo.addItems(["모든 시간대 일치 필요", "2개 이상 일치"])
        self.mtf_mode_combo.setToolTip("모든 시간대(일/주/월)가 일치해야 하는지, 아니면 2개 이상만 일치하면 되는지 선택")
        settings_layout.addWidget(self.mtf_mode_combo, 0, 1)

        # 시장 지수 선택 (참고용 - 실제로는 종목별 자동 감지)
        settings_layout.addWidget(QLabel("시장 지수 (참고):"), 1, 0)
        self.market_index_combo = QComboBox()
        self.market_index_combo.addItems([
            "자동 감지 (권장)",
            "─────── 한국 ───────",
            "^KS11 (KOSPI)",
            "^KQ11 (KOSDAQ)",
        ...
        self.market_index_combo.setToolTip(
            "참고용 표시입니다.\n\n"
            "실제로는 각 종목의 시장을 자동으로 감지합니다:\n"
            "• 한국 (.KS/.KQ) → KOSPI/KOSDAQ\n"
            "• 미국 (기본) → SPY\n"
            "• 스웨덴 (.ST) → OMXS30\n\n"
            "다중 시장 종목을 동시에 스크리닝할 수 있습니다."
        )
        settings_layout.addWidget(self.market_index_combo, 1, 1)
        ...
        self.adv_screening_btn = QPushButton("🚀 고급 스크리닝 시작")
        self.adv_screening_btn.clicked.connect(self.run_advanced_screening)
        self.adv_screening_btn.setStyleSheet("QPushButton { background-color: #9C27B0; color: white; font-weight: bold; padding: 12px; font-size: 14px; }")
        button_layout.addWidget(self.adv_screening_btn)

        self.adv_stop_btn = QPushButton("⏹️ 중지")
        self.adv_stop_btn.clicked.connect(self.stop_screening)
        self.adv_stop_btn.setStyleSheet("QPushButton { background-color: #f44336; color: white; font-weight: bold; padding: 12px; font-size: 14px; }")
    ...
    def apply_advanced_preset(self, preset_name):
        ...
        if preset_name == "🛡️ 안전형":
            # 모든 조건 활성화, 엄격한 설정
            # Phase 1-2: 다중 시간대 + 시장 강도
            self.adv_multi_timeframe.setChecked(True)
            self.adv_market_strength.setChecked(True)
            self.mtf_mode_combo.setCurrentText("모든 시간대 일치 필요")

            # Phase 3: 상대 강도 + 볼륨 프로파일 + 지지선
            self.adv_relative_strength.setChecked(True)
            self.adv_volume_profile.setChecked(True)
            self.adv_support_resistance.setChecked(True)
            ...
            logger.info("📊 안전형 프리셋 적용: 모든 조건(Phase 1-3) + 엄격한 필터 + 기본 매수 조건 활성화")

        elif preset_name == "⚖️ 균형형":
            # 핵심 조건만 활성화, 중간 설정
            # Phase 1-2: 다중 시간대 + 시장 강도
            self.adv_multi_timeframe.setChecked(True)
            self.adv_market_strength.setChecked(True)
            self.mtf_mode_combo.setCurrentText("2개 이상 일치")

            # Phase 3: 상대 강도만 활성화 (볼륨/지지선은 선택적)
            self.adv_relative_strength.setChecked(True)
            self.adv_volume_profile.setChecked(False)
            self.adv_support_resistance.setChecked(False)
            ...
            logger.info("📊 균형형 프리셋 적용: 핵심 조건(Phase 1-2 + 상대강도) + 유연한 필터 + 기본 매수 조건 활성화")

        elif preset_name == "🚀 공격형":
            # 최소 조건, 느슨한 설정
            # Phase 1-2: 비활성화 (시장 타이밍 무시)
            self.adv_multi_timeframe.setChecked(False)
            self.adv_market_strength.setChecked(False)
            self.mtf_mode_combo.setCurrentText("2개 이상 일치")

            # Phase 3: 볼륨 프로파일 + 지지선만 활성화 (기술적 신호 중심)
            self.adv_relative_strength.setChecked(False)
            self.adv_volume_profile.setChecked(True)
            self.adv_support_resistance.setChecked(True)
            ...
            logger.info("📊 공격형 프리셋 적용: 기술적 조건(볼륨+지지선) + 느슨한 필터 + 기본 매수 조건 활성화")

        else:  # 사용자 정의
            logger.info("📊 사용자 정의 모드 선택")

    def run_advanced_screening(self):
        """고급 스크리닝 실행"""
        logger.info("🚀 고급 스크리닝 시작...")

        # 1. 활성화된 조건 수집
        active_conditions = {
            'multi_timeframe': self.adv_multi_timeframe.isChecked(),
            'market_strength': self.adv_market_strength.isChecked(),
            'relative_strength': self.adv_relative_strength.isChecked(),
            'volume_profile': self.adv_volume_profile.isChecked(),
            'support_resistance': self.adv_support_resistance.isChecked(),
        }
        ...
        if not any(active_conditions.values()):
            QMessageBox.warning(
                ...
                "알림",
                "고급 조건을 하나 이상 선택해주세요.\n\n"
                "프리셋을 선택하거나 직접 조건을 체크해주세요."
            )
            return

        # 2. 설정 파라미터 수집
        require_all_timeframes = (self.mtf_mode_combo.currentText() == "모든 시간대 일치 필요")
        market_index_text = self.market_index_combo.currentText()
        market_index = market_index_text.split()[0]  # "SPY (S&P 500)" -> "SPY"

        logger.info(f"활성 조건: {[k for k, v in active_conditions.items() if v]}")
        logger.info(f"다중 시간대 모드: {'모두 일치' if require_all_timeframes else '2개 이상'}")
        logger.info(f"시장 지수: {market_index}")
        ...
        try:
            ...
            # 6. 시장 강도는 각 종목별로 자동 감지
            # (한국, 미국, 스웨덴 종목을 동시에 스크리닝할 수 있으므로 사전 체크 제거)
            # 각 종목 분석 시 해당 시장의 지수를 자동으로 선택하여 체크
            market_strong = True  # 각 종목별로 체크하므로 사전 플래그는 True로 유지

            if active_conditions['market_strength']:
                logger.info("시장 강도 필터 활성화 - 각 종목의 시장 지수를 자동으로 감지합니다")
                logger.info("  한국(KS/KQ) → KOSPI/KOSDAQ, 미국 → SPY, 스웨덴(ST) → OMXS30")
            ...
            if not self.screening_cancelled:
                self.statusbar.showMessage(
                    f'✅ 고급 스크리닝 완료 - 매수후보: {len(buy_candidates)}개, 매도후보: {len(sell_candidates)}개'
                )
    ...
    def analyze_stock_advanced(self, stock_info, active_conditions, require_all_timeframes, market_strong):
        """
        고급 조건을 포함한 종목 분석

        Args:
            stock_info: 종목 정보 딕셔너리
            active_conditions: 활성화된 고급 조건 딕셔너리
            require_all_timeframes: 모든 시간대 일치 필요 여부
            market_strong: 시장 강도 통과 여부
        ...
        try:
            symbol = stock_info['ticker']
            logger.debug(f"🔍 고급 분석 중: {symbol}")

            # 1. 기본 분석 수행 (기존 analyze_stock과 동일)
            ...
            if active_conditions['multi_timeframe']:
                mtf_result, mtf_msg, mtf_details = self.advanced_engine.check_multi_timeframe(
                    symbol,
                    require_all=require_all_timeframes
                )

                if mtf_result is None:
                    logger.debug(f"⚠️ {symbol} - 다중 시간대 데이터 부족")
                    advanced_failed.append("다중시간대_데이터부족")
                elif mtf_result:
                    advanced_signals.append(f"다중시간대✓({mtf_msg})")
                    logger.debug(f"✅ {symbol} - {mtf_msg}: {mtf_details}")
                else:
                    logger.debug(f"❌ {symbol} - {mtf_msg}: {mtf_details}")
                    return None  # 다중 시간대 실패 시 바로 제외

            # 2-2. 시장 강도 확인 (종목 시장에 맞는 지수를 자동 선택)
            if active_conditions['market_strength']:
                ms_result, ms_msg, ms_details = self.advanced_engine.check_market_strength(symbol=symbol)

                if ms_result is None:
                    # 시장 데이터 부족 - 경고만 하고 통과
                    logger.debug(f"⚠️ {symbol} - 시장 데이터 부족: {ms_msg}")
                    advanced_signals.append(f"시장강도(데이터부족)")
                elif ms_result is False:
                    # 시장 약세 - 제외
                    logger.debug(f"❌ {symbol} - {ms_msg}")
                    return None
                else:
                    # 시장 강세 - 통과
                    advanced_signals.append(f"시장강세✓({ms_msg})")
                    logger.debug(f"✅ {symbol} - {ms_msg}")

            # 2-3. 상대 강도 확인 (Phase 3 - 활성화!)
            if active_conditions['relative_strength']:
                ...
                if rs_result is None:
                    logger.debug(f"⚠️ {symbol} - 상대 강도: {rs_msg}")
                elif rs_result:
                    advanced_signals.append(f"상대강도✓({rs_msg})")
                    logger.debug(f"✅ {symbol} - 상대 강도 강함: {rs_details}")
                else:
                    logger.debug(f"❌ {symbol} - 상대 강도 약함: {rs_details}")
                    return None  # 상대 강도 실패 시 제외

            # 2-4. 볼륨 프로파일 확인 (Phase 3 - 활성화!)
            if active_conditions['volume_profile']:
                vp_result, vp_msg, vp_details = self.advanced_engine.check_volume_profile(data)

                if vp_result is True:  # 매수 신호
                    advanced_signals.append(f"볼륨프로파일✓({vp_msg})")
                    logger.debug(f"✅ {symbol} - 볼륨 프로파일 매수: {vp_msg}")
                elif vp_result is False:  # 매도 신호
                    logger.debug(f"❌ {symbol} - 볼륨 프로파일 매도: {vp_msg}")
                    return None  # 매도 구간이면 제외

            # 2-5. 지지/저항선 확인 (Phase 3 - 활성화!)
            if active_conditions['support_resistance']:
                ...
                if sr_result is True:  # 지지선 근처
                    advanced_signals.append(f"지지선근처✓({sr_msg})")
                    logger.debug(f"✅ {symbol} - 지지선 근처: {sr_msg}")
                elif sr_result is False:  # 저항선 근처
                    logger.debug(f"❌ {symbol} - 저항선 근처: {sr_msg}")
                    # 저항선 근처는 매수에는 불리하지만 제외하지는 않음
                    pass

            # 3. 기본 매수/매도 조건 체크 (기존 로직)
            buy_signals = []
            sell_signals = []

            # 기본 매수 조건들 (기존 analyze_stock과 동일)
            ...
            # 기본 매도 조건들 (기존 analyze_stock과 동일)
            if self.tech_sell.isChecked():
                if (current['MA60'] < current['MA120'] and
                    current['Close'] < current['MA60'] * 0.97):
                    sell_signals.append("데드크로스+하향이탈")

            if self.profit_sell.isChecked():
                # 수익/손절 조건 (실제 매수가 필요)
                pass
            ...
            if self.volume_sell.isChecked():
                if (current['Volume_Ratio'] < 0.5 and
                    current['RSI'] < prev['RSI'] - 5):
                    sell_signals.append("거래량급감+모멘텀약화")
            ...
            if buy_signals:
                ...
                result = {
                    'symbol': symbol,  # 'ticker' → 'symbol' 변경
                    ...
                    'ma60': current['MA60'],
                    'ma120': current['MA120'],
                    'recommendation': len(buy_signals + advanced_signals) * 20,  # 신호개수 기반 추천도
                }

                # 추세 분석 결과 추가
                if trend_analysis:
                    result['trend'] = trend_analysis.get('trend', 'N/A')
                    result['timing'] = trend_analysis.get('timing', 'N/A')

                logger.info(f"✅ 매수 후보: {symbol} - {', '.join(all_signals)}")
                return result

            elif sell_signals:
                all_signals = sell_signals + advanced_signals

                # 섹터 정보 가져오기 (이미 위에서 import됨)
                ...
                result = {
                    ...
                    'holding_period': '미상',
                    'risk': len(sell_signals + advanced_signals) * 25,  # 신호 개수 기반 위험도
                }

                # 추세 분석 결과 추가
                if trend_analysis:
                    result['trend_direction'] = trend_analysis.get('trend_direction', '분석불가')
                    result['trend_score'] = trend_analysis.get('trend_score', 0)
                    result['sell_timing'] = trend_analysis.get('sell_timing', {}).get('grade', '대기')

                logger.info(f"✅ 매도 후보: {symbol} - {', '.join(all_signals)}")
                return result

            return None

        except Exception as e:
            logger.error(f"고급 분석 오류 {symbol}: {e}", exc_info=True)
            return None
